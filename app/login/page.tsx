...
import { Eye, EyeOff, Sparkles, KeyRound, Mail, X, Send, ArrowRight } from 'lucide-react';
...
export default function LoginPage() {
  ...
  // Forgot / Reset Password Request Modal States (Handled strictly by Admin)
  const [showForgotModal, setShowForgotModal] = useState(false);
  const [forgotPrimaryEmail, setForgotPrimaryEmail] = useState('');
  const [sendingReset, setSendingReset] = useState(false);
  const [resetSuccessMsg, setResetSuccessMsg] = useState('');

  // 1. Google OAuth (Strictly for Manager/Admin)
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        // Register standard employee account without self-recovery requirements
  ...
  // 3. Admin Request Handler for Password Reset
  const handleAdminResetRequest = async (e: React.FormEvent) => {
    e.preventDefault();
    setResetSuccessMsg('');
    setErrorMsg('');

    const targetPrimary = forgotPrimaryEmail.trim().toLowerCase();
    if (!targetPrimary) {
    ...
    try {
      // Dispatches password reset request notification directly to Admin
      const mailRes = await fetch('/api/send-mail', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          officialEmail: targetPrimary,
          isResetRequestToAdmin: true,
        }),
      });

      const result = await mailRes.json();
      if (!mailRes.ok) {
        throw new Error(result.error || 'Failed to send reset request to admin.');
      }

      setResetSuccessMsg('Password reset request has been successfully dispatched to the Administrator.');
      setTimeout(() => {
        setShowForgotModal(false);
        setResetSuccessMsg('');
        setForgotPrimaryEmail('');
      }, 3500);
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-4 relative">
      <div className="w-full max-w-md bg-[#161b22] border border-gray-800 rounded-2xl p-6 md:p-8 shadow-2xl">
        ...
        <p className="text-xs text-gray-400 mb-6">
          {role === 'manager'
            ...
            : 'Sign in securely with your official email and password.'}
        </p>

        {/* Google Workspace OAuth Button (Manager only) */}
        {role === 'manager' && (
          <button
            type="button"
        ...
        {/* Employee Only: Clean Password Form (No Recovery Email field) */}
        {role === 'employee' && (
          <>
      ...
      {/* ================= ADMIN PASSWORD RESET REQUEST MODAL ================= */}
      {showForgotModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#161b22] border border-gray-800 rounded-2xl w-full max-w-md flex flex-col shadow-2xl overflow-hidden p-6">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400">
                    Send a direct password recovery request to the Admin
                  </p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleAdminResetRequest} className="space-y-4 text-xs">
              <div>
                <label className="block text-gray-400 uppercase font-bold tracking-wider mb-1.5">
                  Official Exampur Email ID *
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    required
                    placeholder="name@exampur.com"
                    value={forgotPrimaryEmail}
                    onChange={(e) => setForgotPrimaryEmail(e.target.value)}
                ...
                <p className="text-[11px] text-gray-400 mt-2 leading-relaxed">
                  Your request will be routed directly to the Administrator for verification and password reset.
                </p>
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                >
                  ...
                  {sendingReset ? 'Sending Request...' : 'Send Request to Admin'}
                </button>
              </div>
            </form>
          </div>
