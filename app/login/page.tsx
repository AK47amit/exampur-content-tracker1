...
export default function LoginPage() {
  ...
  // Forgot / Reset Password Multi-Step Modal States
  const [showForgotModal, setShowForgotModal] = useState(false);
  const [forgotStep, setForgotStep] = useState<1 | 2>(1); // Step 1: Primary Email, Step 2: Recovery Email
  ...
  // 1. Google OAuth (Strictly for Manager/Admin now as requested)
  ...
  // 3. Multi-Step Forgot Password / Recovery Handler via Nodemailer
  const handlePasswordResetSubmit = async (e: React.FormEvent) => {
    ...
    if (forgotStep === 1) {
      if (!forgotPrimaryEmail.trim()) {
        alert('Please enter your primary Exampur email address.');
        return;
      }
      setForgotStep(2);
      return;
    ...
    const targetRecovery = forgotRecoveryEmail.trim().toLowerCase();
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] flex items-center justify-center p-4 relative">
      <div className="w-full max-w-md bg-[#161b22] border border-gray-800 rounded-2xl p-6 md:p-8 shadow-2xl">
        ...
        {/* Google Workspace OAuth Button (Only shown for Manager tab now) */}
        {role === 'manager' && (
          <button
            type="button"
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">
                    {forgotStep === 1 ? 'Step 1: Enter Primary ID' : 'Step 2: Enter Recovery Email'}
                  </h3>
                  <p className="text-[11px] text-gray-400">
                    {forgotStep === 1 ? 'Provide your official Exampur email address' : 'Provide your registered personal recovery email'}
                  </p>
                </div>
              </div>
              <button
              ...
              {forgotStep === 1 ? (
                <div>
                  <label className="block text-gray-400 uppercase font-bold tracking-wider mb-1.5">
                    Official Exampur ID (Primary Mail) *
                  </label>
                  <div className="relative">
                  ...
                  <p className="text-[11px] text-gray-400 mt-2 leading-relaxed">
                    Enter your official account identifier. In the next step, we will verify your recovery destination.
                  </p>
                </div>
              ) : (
                <div>
                  ...
                  <p className="text-[11px] text-gray-400 mt-2 leading-relaxed">
                    We will dispatch secure password reset tokens strictly to this recovery inbox for <span className="text-[#ff5722] font-semibold">{forgotPrimaryEmail}</span>.
                  </p>
                </div>
              )}
